...
class DatenaufzeichnerGUI:
    ...
    def __init__(self, hauptfenster):
       ...
       # Logging initialisieren und Log-Message rausschreiben
        self.logger = logging.getLogger(__name__ + ".DatenaufzeichnerGUI")
        self.logger.info("Datenaufzeichner GUI gestartet")
        ...
        ttk.Label(master=unterrahmen1, text="Datenaufzeichner Einstellungen",
                  style="BlockLabel2.TLabel").grid(column=1, row=1, columnspan=3, sticky="NW")
        ttk.Label(master=unterrahmen1, text="Interval (s)", style="BlockLabel.TLabel").grid(
            column=1, row=2, sticky="NW")  # Intervall
        ...
        self.dateiname.set("daten_logger.log")
        ...
        ttk.Label(master=unterrahmen2, text="Datenaufzeichner Steuerung",
                  style="BlockLabel2.TLabel").grid(column=1, row=1, columnspan=2, sticky="NW")
        ...
        ttk.Label(master=unterrahmen2, textvariable=self.gen_status,
                  style="BlockStatusLabelDat.TLabel").grid(column=3, row=2, columnspan=2, sticky="NE")
    ...
    def exrahiere_optionen(self):
        """ Dictionary mit Optionen für Datenaufzeichner erstellen """
...
class Datenaufzeichner:
    ...
    def schreibe_daten(self):
        ...
        de_zugriff = dzg.DateiZugriff(self.konfig["DIGEIN"], self.konfig["DIGMAXLAENGE"])  # Digitale Eingänge rausschreiben
        ...
        da_zugriff = dzg.DateiZugriff(self.konfig["DIGAUS"], self.konfig["DIGMAXLAENGE"])  # Digitale Ausgänge rausschreiben
        da_daten = da_zugriff.lesen_alle()
        for i in range(self.konfig["DIGMAXLAENGE"]):
            zeile = zeile + ";"
            zeile = zeile + da_daten[i]
        aa_zugriff = dzg.DateiZugriff(self.konfig["ANAAUS"], self.konfig["ANAMAXLAENGE"])  # Analoge Ausgänge rausschreiben
