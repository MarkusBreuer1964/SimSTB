...
def test_dig_ein():
    ...
    daten_dig[7] = 0
    dzg.DateiZugriff(kfg.Konfig.DIGEIN, kfg.Konfig.DIGMAXLAENGE).schreiben_alle(daten_dig)
...
def test_dig_aus():
    ...
    daten_dig = dzg.DateiZugriff(kfg.Konfig.DIGAUS, kfg.Konfig.DIGMAXLAENGE).lesen_alle()
    # Ergebnisse prüfen
...
def test_ana_ein():
    ...
    daten_ana = ["0.0"] * kfg.Konfig.ANAMAXLAENGE
    daten_ana[3] = 123.456
    dzg.DateiZugriff(kfg.Konfig.ANAEIN, kfg.Konfig.ANAMAXLAENGE).schreiben_alle(daten_ana)
...
def test_ana_aus():
    ...
    sim.ana_aus(7, 0.0)
    # Daten aus Datei zurücklesen
    daten_ana = dzg.DateiZugriff(kfg.Konfig.ANAAUS, kfg.Konfig.ANAMAXLAENGE).lesen_alle()
    # Ergebnisse prüfen
